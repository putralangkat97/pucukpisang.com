<script setup>
const { audio, copiedText, isTranslationOpen } = defineProps({
    audio: {
        type: Object,
        default: null
    },
    ...
    <details class="collapse collapse-arrow bg-base-100 border-base-300 border rounded-2xl" :open="isTranslationOpen">
        ...
        <div class="collapse-content relative">
            <div class="absolute right-12 -top-10 z-10">
                <button @click.stop="emit('copy-text', audio.translations, 'translation')" class="btn btn-xs btn-neutral/80 mr-4"
                    :class="{ 'btn-neutral': copiedText === 'translation' }">
                    {{ copiedText === 'translation' ? 'Copied!' : 'Copy' }}
                </button>
                <a :href="`/audio/results/${audio.id}/translation/download`" class="btn btn-xs btn-primary">
                    Download
                </a>
            </div>
            <pre
                class="bg-base-300/40 p-4 rounded-lg text-base-content/80 whitespace-pre-wrap font-sans">{{ audio.translations }}</pre>
        </div>
    </details>
</template>
